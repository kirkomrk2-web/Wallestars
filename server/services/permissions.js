...
export class WallestarsPermissions {
    ...
    validateApiKey(key) {
        ...
        if (key.startsWith('sk-ant-')) {
            // Check against the server's configured key
            // This ensures only the owner (who knows the key) gets admin access
        ...
        // Validate Wallestars Key (for agents/integrations)
        if (key.startsWith('ws-')) {
            // Check against configured Wallestars key
            if (process.env.WALLESTARS_API_KEY && key === process.env.WALLESTARS_API_KEY) {
